...
class ISBI_Loader(Dataset):
    ...
    def __getitem__(self, index):
        ...
        #随机进行数据增强，为2时不处理
        angleCote = random.choice([1,0,-1,2])
        # if angleCote != 2:
        #     #rate = random.choice([0.5,0.8,0.9,1.0])
        image1 = self.augment(image1,angleCote)
        image2 = self.augment(image2,angleCote)
        label = self.augment(label,angleCote)
...
if __name__ == "__main__":
    ...
    print("*********")
    isbi_dataset = ISBI_Loader(data_path="data/levir-cd/train2/",
